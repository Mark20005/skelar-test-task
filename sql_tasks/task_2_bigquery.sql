...
ON T.user_bk = S.user_bk AND T._valid_to = TIMESTAMP('9999-12-31 23:59:59') -- assuming that this 'magic' date used to mark active records

-- an SCD2 attribute changed(marking record as historical with setting _valid_to = load_timestamp)
WHEN MATCHED AND (T.country <> S.country OR T.city <> S.city) THEN
  UPDATE SET T._valid_to = load_timestamp
...
SELECT
    ...
    S.country,
    S.city,
    -- setting _valid_from from our variable
    load_timestamp AS _valid_from,
    TIMESTAMP('9999-12-31 23:59:59') AS _valid_to
FROM `test.stg_user` S
LEFT JOIN `test.dim_user` T
  ON S.user_bk = T.user_bk
  AND T._valid_to = TIMESTAMP('9999-12-31 23:59:59')
WHERE
